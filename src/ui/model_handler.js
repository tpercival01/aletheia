...
export class AI_MODEL{
  ...
  async initialize(){
    ...
    this.initPromise = this._load_model();
  ...
  async processText(textChunks) {
    if(!this.isInitialized) await this.initialize();

    const results = [];
    for (const chunk of textChunks){
      const confidence = await this.detectAI(chunk.text);
      results.push({
        ...chunk,
        confidence,
        isAI: confidence > 0.5
  ...
  async detectAI(text) {
    ...
    try {
      ...
      const prediction = this.model.predict(inputs.input_ids);
